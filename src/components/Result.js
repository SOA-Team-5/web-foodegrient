...
function Result() {
  ...
  async function handleLikeBtn(id, index) {
    const res = await Axios.get(
      `https://foodegrient-api.herokuapp.com/api/v1/like?id=${id}`
    );
    let tempMenuData = menuData;
    tempMenuData[index].likes += 1;
    setMenuData(tempMenuData);
    setMenuData(prevList => {
        prevList[index].checked = !prevList[index].checked // 这里数据引用地址的值同样被修改
        return [...prevList]})
  }

  async function handleUnlikeBtn(id, index) {
    const res = await Axios.get(
      `https://foodegrient-api.herokuapp.com/api/v1/unlike?id=${id}`
    );
    let tempMenuData = menuData;
        ...
        <div className="card-body">
          ...
          <li class="list-group-item d-flex justify-content-between align-items-start">
            <div class="ms-2 me-auto">
              ...
              Likes: {menuData[i].likes}{" "}
            </div>
            <div onClick={()=>{handleLikeBtn(menuData[i].recipe_id, i)}}>
              <svg
          ...
          <li class="list-group-item d-flex justify-content-between align-items-start">
            <div class="ms-2 me-auto">
              <div class="fw-bold"></div>
              Unlikes: {menuData[i].unlikes}{" "}
            </div>
            <div onClick={()=>{handleUnlikeBtn(menuData[i].recipe_id, i)}}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
